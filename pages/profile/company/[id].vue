<template>
    <div class="iwq-container" v-if="company">
        <section v-if="!pending" class="profile-container">
            <div class="profile-section main">
                <div class="profile-picture" :style="getImageFromDirectus(company.Logo, 'png')"></div>
                <div>
                    <h2>{{ company.Unternehmen }}</h2>
                    <p class="profile-info-region">{{ company.Ort }}</p>
                </div>
                <div class="profile-info-bio">
                    <p><b>Das sind wir:</b></p>
                    <p>{{ company.Biografie }}</p>
                </div>

                <div class="custom-button-holder">
                    <button class="custom-button small" @click="makeNewChat(company.id)">{{company.Vorname }} kontaktieren
                    </button>
                </div>
            </div>

            <div class="profile-section" v-if="company.Skills">
                <h2 class="profile-section-headline">Gesuchte Skills</h2>
                <ul class="iwq-checklist split-3">
                  <li v-for="skillEl in company.Skills">{{ skillEl.skill }}</li>
                </ul>
            </div>
        </section>
    </div>
